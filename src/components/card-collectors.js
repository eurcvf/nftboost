...
export function CardCollectors({ image, name, price, percent }) {
  return (
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            <h4>{name}</h4>
            <p className="flex items-center gap-1 text-sm text-white text-opacity-70">Preço mínimo <span className="flex items-center gap-1 font-semibold text-sm"><Image src={IconETH} alt="Icon ETH"/> {price} BTC</span></p>
          </div>
        </div>
      </div>
